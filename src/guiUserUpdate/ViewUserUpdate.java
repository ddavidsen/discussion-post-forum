...
import javafx.scene.control.PasswordField;         // <-- added
...
public class ViewUserUpdate {
	...
	// New password UI elements (added so controller can read the fields)
	// These are protected static so ControllerUserUpdate can access them (same style as other views).
	protected static PasswordField text_NewPassword = new PasswordField();         // <-- added
	protected static PasswordField text_ConfirmPassword = new PasswordField();     // <-- added
	private static Button button_SaveNewPassword = new Button("Save New Password"); // <-- added
	...
	private static Button button_ProceedToAdminHomePage = new Button("Login again to proceed to User Home Page");
	...
	private ViewUserUpdate() {
        ...
        /* 
         * Password update UI handling:
         * The view contains two PasswordFields and a Save button that are initially hidden.
         * When the user clicks "Update Password", the fields are shown and the Save button
         * calls ControllerUserUpdate.doSaveNewPassword(...) with the current stage and user.
         * This keeps the UI changes localized to the view and lets the controller perform
         * validation and the database update.
         */
        // Initially hidden — will be shown when Update Password is clicked
        ...
        text_NewPassword.setManaged(false);
        text_ConfirmPassword.setVisible(false);
        text_ConfirmPassword.setManaged(false);
        button_SaveNewPassword.setVisible(false);
        button_SaveNewPassword.setManaged(false);

        // When Update Password clicked, reveal the password fields and save button
        button_UpdatePassword.setOnAction((event) -> {
        	// Position the fields (use helper to style)
        	setupTextUI(text_NewPassword, "Arial", 14, 300, Pos.BASELINE_LEFT, 50, 470, true);
        	setupTextUI(text_ConfirmPassword, "Arial", 14, 300, Pos.BASELINE_LEFT, 50, 510, true);
        	setupButtonUI(button_SaveNewPassword, "Dialog", 14, 200, Pos.CENTER, 500, 503);
        	// Show them
        	text_NewPassword.setVisible(true);
        	text_NewPassword.setManaged(true);
        	text_ConfirmPassword.setVisible(true);
        	text_ConfirmPassword.setManaged(true);
        	button_SaveNewPassword.setVisible(true);
        	button_SaveNewPassword.setManaged(true);
        	// Hook up save action to controller — passes the stage and the current user
        	button_SaveNewPassword.setOnAction((evt) -> {
        		ControllerUserUpdate.doSaveNewPassword(theStage, theUser);
        	});
        });
        ...
        // If the user is the first admin, require them to login again before going to home
        refreshButtons();
        ...
        theRootPane.getChildren().addAll(
        		...
        		button_ProceedToUserHomePage, button_ProceedToAdminHomePage,
                // Add new password fields and save button (initially hidden)
                text_NewPassword, text_ConfirmPassword, button_SaveNewPassword);
	}
	
	/**********
	* <p> Method: refreshButtons() </p>
	*
	* <p> Description: This method sets up the button to proceed to this user's home page. It
	* is dependent on if the current user is the first user. If the user is the first admin, 
	* require them to login again before going to home.</p>
	*
	*/
	private static void refreshButtons() {
		boolean firstAdminDone = FoundationsMain.database.isFirstAdminSetupComplete();
		     
        button_ProceedToAdminHomePage.setVisible(false);
        button_ProceedToAdminHomePage.setManaged(false);
        button_ProceedToUserHomePage.setVisible(false);
        button_ProceedToUserHomePage.setManaged(false);
       
        if (!firstAdminDone) {
        	button_ProceedToAdminHomePage.setVisible(true);
        	button_ProceedToAdminHomePage.setManaged(true);
        	setupButtonUI(button_ProceedToAdminHomePage, "Dialog", 18, 350, 
        		Pos.CENTER, width/2-185, 450);
        button_ProceedToAdminHomePage.setOnAction((event) -> 
        	{FoundationsMain.database.setFirstAdminSetupComplete();
        		ControllerUserUpdate.performLogout();
        		System.out.println(FoundationsMain.database.isFirstAdminSetupComplete());});
        }
	   
	    else {
	    	button_ProceedToUserHomePage.setVisible(true);
	    	button_ProceedToUserHomePage.setManaged(true);
	 ...
	 */
	private static void setupTextUI(javafx.scene.control.TextField t, String ff, double f, double w, Pos p, double x, double y, boolean e){
