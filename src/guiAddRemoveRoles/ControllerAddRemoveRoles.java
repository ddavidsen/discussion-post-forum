...
public class ControllerAddRemoveRoles {
	...
	protected static void repaintTheWindow() {
		...
		ViewAddRemoveRoles.theStage.setTitle("Admin Operations Page");
	...
	protected static void refreshUserList() {
	    ...
	    ViewAddRemoveRoles.combobox_SelectUser.setItems(
	        FXCollections.observableArrayList(userList)
	    );

	    if (!userList.isEmpty()) {
	    	ViewAddRemoveRoles.combobox_SelectUser.getSelectionModel().select(0);
	    	ViewAddRemoveRoles.theSelectedUser = ViewAddRemoveRoles.combobox_SelectUser.getSelectionModel().getSelectedItem();
	    } else {
	    	ViewAddRemoveRoles.theSelectedUser = "<Select a User>";
	    }
	}
	...
	protected static void performRemoveRole() {
		...
		//If the current admin has selected themselves, and they try to remove admin, present an error
		if (ViewAddRemoveRoles.theSelectedUser.equals(ViewAddRemoveRoles.theUser.getUserName())
			...
			ViewAddRemoveRoles.cantRemoveAlert.setTitle("*** ERROR ***");
			ViewAddRemoveRoles.cantRemoveAlert.setHeaderText("Remove Role Issue");
			ViewAddRemoveRoles.cantRemoveAlert.setContentText("You cannot remove your own admin role!");
			ViewAddRemoveRoles.cantRemoveAlert.showAndWait();
		}
		
		// If the selection is the list header (e.g., "<Select a role>") don't do anything
		else if (ViewAddRemoveRoles.theRemoveRole.compareTo("<Select a role>") != 0) {
